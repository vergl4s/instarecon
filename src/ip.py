...
class IP(object):
            ...
            if "nets" in self.whois_ip:
                if self.whois_ip["nets"]:
                    ...
                    for net in self.whois_ip["nets"]:
                        for cidr in net["cidr"].split(","):
                            if cidr:
                                if sys.version_info[0] == 2:
                                    cidr = unicode(cidr)

                                cidrs.append(ipa.ip_network(cidr.rstrip().lstrip()))
    ...
    def print_whois_ip(self):
        if self.whois_ip:
            ...
            for key, val in sorted(self.whois_ip.items()):
            ...
            for key, net in enumerate(self.whois_ip["nets"]):
                ...
                if net["range"]:
                    result = "\n\t".join(
                        [result, "range" + ": " + net["range"].replace("\n", ", ")]
                    )
                if net["name"]:
                    result = "\n\t".join([result, "name" + ": " + net["name"].replace("\n", ", ")])
                if net["description"]:
                    result = "\n\t".join(
                        [result, "description" + ": " + net["description"].replace("\n", ", ")]
                    )
                if net["handle"]:
                    result = "\n\t".join(
                        [result, "handle" + ": " + net["handle"].replace("\n", ", ")]
                    )
                result = "\n\t".join([result, ""])
                if net["address"]:
                    result = "\n\t".join(
                        [result, "address" + ": " + net["address"].replace("\n", ", ")]
                    )
                if net["city"]:
                    result = "\n\t".join([result, "city" + ": " + net["city"].replace("\n", ", ")])
                if net["state"]:
                    result = "\n\t".join(
                        [result, "state" + ": " + net["state"].replace("\n", ", ")]
                    )
                if net["postal_code"]:
                    result = "\n\t".join(
                        [result, "postal_code" + ": " + net["postal_code"].replace("\n", ", ")]
                    )
                if net["country"]:
                    result = "\n\t".join(
                        [result, "country" + ": " + net["country"].replace("\n", ", ")]
                    )
                result = "\n\t".join([result, ""])
                if "abuse_emails" in net:
                    result = "\n\t".join(
                        [result, "abuse_emails" + ": " + net["abuse_emails"].replace("\n", ", ")]
                    )
                if "tech_emails" in net:
                    result = "\n\t".join(
                        [result, "tech_emails" + ": " + net["tech_emails"].replace("\n", ", ")]
                    )
                if "misc_emails" in net:
                    result = "\n\t".join(
                        [result, "misc_emails" + ": " + net["misc_emails"].replace("\n", ", ")]
                    )
                result = "\n\t".join([result, ""])
