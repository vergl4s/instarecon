...
def dns_lookup_manager(target, lookup_type, suppress_warning=False):
    ...
    while tries < dns_maximum_retries:
        try:
            ...
            message = (
                lookup_type + " lookup failed for " + target + " - " + str(e.__class__.__name__)
            )
            ...
            if tries < dns_maximum_retries:
                ...
                logging.info(
                    str(dns_maximum_retries)
                    + " timeouts resolving "
                    + target
                    + ". Internet connection alright?"
                )
...
def _update_google_results(new_google_results, results_dictionary):
    ...
    for url in new_google_results:
        ...
        if len(temp) > 1:
            ...
            logging.debug(
                "Found host " + g_host + " with port " + temp[1] + ". New pathname is " + g_pathname
            )
...
def test_internet_connection():
    ...
    s.settimeout(2)
    possible_targets = ["8.8.8.8", "8.8.4.4", "139.130.4.5"]
    try:
