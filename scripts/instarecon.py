...
class InstaRecon(object):
    ...
    def __init__(self, nameserver=None, timeout=None, shodan_key=None, verbose=0):
    ...
    def populate(self, user_supplied_list):
        ...
        if not self.targets:
            ...
            print(
                "# Scanning {}/{} hosts".format(
                    str(len(self.targets)), str(len(user_supplied_list))
                )
            )
    ...
    def scan_host_whois(self, host):
        ...
        if host.whois_domain:
            print("[*] Whois domain:")
            print(host.whois_domain.decode())
    ...
    def scan_host_shodan(self, host):
        ...
        if lookup.shodan_key:
            ...
            print(
                "# Can't do Shodan lookups without a key (pass one with -s or with unix environment variable SHODAN_KEY)"
            )
        print("")
    ...
    def scan_host_google(self, host):
        ...
        if host.domain:
            ...
            if host.google_subdomains:
                ...
                logging.error(
                    "No subdomains found in Google. If you are scanning a lot, Google might be blocking your requests."
                )
            print("")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "targets",
        nargs="+",
        help="targets to be scanned - can be a domain (google.com), an IP (8.8.8.8) or a network range (8.8.8.0/24)",
    )
    parser.add_argument("-o", "--output", required=False, nargs="?", help="output filename as csv")
    parser.add_argument(
        "-n", "--nameserver", required=False, nargs="?", help="alternative DNS server to query"
    )
    parser.add_argument(
        "-s",
        "--shodan_key",
        required=False,
        nargs="?",
        help="shodan key for automated port/service information (SHODAN_KEY environment variable also works for this)",
    )
    parser.add_argument(
        "-t",
        "--timeout",
        ...
        nargs="?",
        type=float,
        help="timeout for DNS lookups (default is 2s)",
    )
    parser.add_argument(
        "-v",
        "--verbose",
        action="count",
        default=0,
        help="verbose errors (-vv or -vvv for extra verbosity)",
    )
